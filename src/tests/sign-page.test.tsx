...
const mockDeep = <T extends NonNullable<unknown>>() => ({} as T);
...
describe('로그인 페이지 링크 기능', () => {
  ...
  test('로그인에 성공하였을 시 main route로 이동한다', async () => {
    ...
    mockedSignIn.mockResolvedValue(mockDeep<UserCredential>());

    const { user } = render(<SignIn />, { route: '/signin' });
    const email = screen.getByPlaceholderText('이메일');
    const password = screen.getByPlaceholderText('비밀번호');
    const submitButton = screen.getByRole('button', { name: '로그인' });
    ...
    console.log(jest.mocked(signInWithEmailAndPassword).mock.results);
    expect(jest.mocked(signInWithEmailAndPassword).mock.calls).toHaveLength(1);
